...
Write-Host ""
Write-Host "It also needs DEVPLATFORM_RELEASES_ROOT set to the exact same absolute path"
Write-Host "devplatform.exe uses for its VersionStore (DataDir\releases). This is the one"
Write-Host "directory tree a physical path is ever allowed to point into - it stops a"
Write-Host "compromised devplatform.exe from repointing an allowed site at an unrelated"
Write-Host "folder on this server. If it is empty or unset, iishelper rejects every request."
Write-Host ""
Write-Host "IMPORTANT: Windows Services do NOT inherit a logged-in user's environment. Setting"
Write-Host "DEVPLATFORM_IISHELPER_SDDL, DEVPLATFORM_ALLOWED_SITES_FILE, or DEVPLATFORM_RELEASES_ROOT"
Write-Host "with a plain user-scoped 'setx' or in a PowerShell profile will NOT be visible to"
Write-Host "this service. All three variables must be set as machine-scoped environment"
Write-Host "variables - System Properties > Environment Variables > System variables (not"
Write-Host "User variables), or:"
Write-Host ""
...
Write-Host "  [Environment]::SetEnvironmentVariable('DEVPLATFORM_RELEASES_ROOT', '<path>', 'Machine')"
Write-Host ""
